...
$LibDir      = Join-Path $ProjectRoot "Project\lib\*"
...
Write-Host "[BUILD] Compiling Java sources with library dependencies..." -ForegroundColor Cyan
$Sources = Get-ChildItem -Path $SrcDir -Filter "*.java" | ForEach-Object { $_.FullName }

javac -cp $LibDir -d $OutDir @Sources
...
$RuntimeClasspath = "$OutDir;$LibDir"

Set-Location $ProjectRoot
java -cp $RuntimeClasspath Main
